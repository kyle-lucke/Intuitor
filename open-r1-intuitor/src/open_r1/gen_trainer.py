...
def compute_generalized_entropy(probs, gamma):
    return torch.sum(torch.pow(probs * (1-probs), gamma), dim=-1)
...
class GENTrainer(Trainer):
    ...
    def _get_per_token_self_certainty(
        ...
        for i in range(0, input_ids.size(0), batch_size):
            ...
            # Keep only the logits for the completion tokens
            completion_logits = logits[:, -logits_to_keep:, :]  # (B, L_keep, V)

            # Normalize to probabilities for GEN:
            probs = torch.nn.functional.softmax(completion_logits, dim=-1)

            # TODO: create self.gamma and pass as an input parameter
            gen_values = compute_generalized_entropy(probs, gamma=self.gamma)
            
            all_gen.append(gen_values)
            ...
            # print(sce_chunk.shape)
            # print(sce_values.shape)
            # exit()
